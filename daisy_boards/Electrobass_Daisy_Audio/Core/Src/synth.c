...
void audioInitSynth()
{
	...
	for (int v = 0; v < NUM_STRINGS_PER_BOARD; v++)
	{
		...
		for (int i = 0; i < NUM_FILT; i++)
		{
			...
			tExpSmooth_init(&filterCutoffSmoother[i][v], 64.0f, 0.002f, &leaf);
		}
...
float freqToSmoothMemory[NUM_OSC][NUM_STRINGS_PER_BOARD];
...
void __ATTR_ITCMRAM oscillator_tick(float note, int string)
{
	...
	for (int osc = 0; osc < oscToTick; osc++)
	{
		if (oscOn[osc])
		{
			...
			if ((freqToSmooth > (freqToSmoothMemory[osc][string] + 1.0f)) || (freqToSmooth < (freqToSmoothMemory[osc][string] - 1.0f)) )
			{
				tExpSmooth_setVal(&pitchSmoother[osc][string], freqToSmooth);
			}
			tExpSmooth_setDest(&pitchSmoother[osc][string], freqToSmooth);
			freqToSmoothMemory[osc][string] = freqToSmooth;
