...
***************************************/
    ...
    static const uint8 CYCODE USBMIDI_MIDI_SIZE[] = {
...
***************************************/
        ...
        * USBMIDI_MIDI_EP_Init() function.*/
        volatile uint16 USBMIDI_midiInPointer;                            /* Input endpoint buffer pointer */
    #else
        volatile uint8 USBMIDI_midiInPointer;                             /* Input endpoint buffer pointer */
    ...
    volatile uint8 USBMIDI_midi_in_ep;
    ...
    uint8 USBMIDI_midiInBuffer[USBMIDI_MIDI_IN_BUFF_SIZE];       /* Input endpoint buffer */
    ...
    volatile uint8 USBMIDI_midi_out_ep;                                   /* Output endpoint number */
    ...
    uint8 USBMIDI_midiOutBuffer[USBMIDI_MIDI_OUT_BUFF_SIZE];     /* Output endpoint buffer */
#endif /* (USBMIDI_MIDI_OUT_BUFF_SIZE > 0) */

#if (USBMIDI_MIDI_EXT_MODE >= USBMIDI_ONE_EXT_INTRF)

    static USBMIDI_MIDI_RX_STATUS USBMIDI_MIDI1_Event;            /* MIDI RX status structure */
    static volatile uint8 USBMIDI_MIDI1_TxRunStat;                         /* MIDI Output running status */
    ...
    volatile uint8 USBMIDI_MIDI1_InqFlags;                                 /* Device inquiry flag */

    #if (USBMIDI_MIDI_EXT_MODE >= USBMIDI_TWO_EXT_INTRF)
        static USBMIDI_MIDI_RX_STATUS USBMIDI_MIDI2_Event;        /* MIDI RX status structure */
        static volatile uint8 USBMIDI_MIDI2_TxRunStat;                     /* MIDI Output running status */
        /** See description of \ref USBMIDI_MIDI1_InqFlags*/
        volatile uint8 USBMIDI_MIDI2_InqFlags;                             /* Device inquiry flag */
...
#if (USBMIDI_ENABLE_MIDI_API != 0u)
/*******************************************************************************
* Function Name: USBMIDI_MIDI_Init
****************************************************************************//**
*
...
* \globalvars
*
*  \ref USBMIDI_midiInBuffer: This buffer is used for saving and combining
*    the received data from UART(s) and(or) generated internally by
*    PutUsbMidiIn() function messages. USBMIDI_MIDI_IN_EP_Service()
*    function transfers the data from this buffer to the PC.
*
*  \ref USBMIDI_midiOutBuffer: This buffer is used by the
*    USBMIDI_MIDI_OUT_Service() function for saving the received
*    from the PC data, then the data are parsed and transferred to UART(s)
*    buffer and to the internal processing by the
*
*  \ref USBMIDI_callbackLocalMidiEvent function.
*
*  \ref USBMIDI_midi_out_ep: Used as an OUT endpoint number.
*
*  \ref USBMIDI_midi_in_ep: Used as an IN endpoint number.
*
*   \ref USBMIDI_midiInPointer: Initialized to zero.
*
* \sideeffect
...
void USBMIDI_MIDI_Init(void) 
{
   ...
   USBMIDI_midiInPointer = 0u;
        ...
        USBMIDI_LoadInEP(USBMIDI_midi_in_ep, USBMIDI_midiInBuffer,
                                                               USBMIDI_MIDI_IN_BUFF_SIZE);
    #endif  /* (USBMIDI_MIDI_IN_BUFF_SIZE > 0) */

    #if (USBMIDI_MIDI_OUT_BUFF_SIZE > 0)
        /* Provide buffer for OUT endpoint. */
        (void)USBMIDI_ReadOutEP(USBMIDI_midi_out_ep, USBMIDI_midiOutBuffer,
                                                                       USBMIDI_MIDI_OUT_BUFF_SIZE);
    #endif /* (USBMIDI_MIDI_OUT_BUFF_SIZE > 0) */
#endif /* (USBMIDI_EP_MANAGEMENT_DMA_AUTO) */

#if (USBMIDI_MIDI_OUT_BUFF_SIZE > 0)
    USBMIDI_EnableOutEP(USBMIDI_midi_out_ep);
#endif /* (USBMIDI_MIDI_OUT_BUFF_SIZE > 0) */

    /* Initialize the MIDI port(s) */
#if (USBMIDI_MIDI_EXT_MODE >= USBMIDI_ONE_EXT_INTRF)
    USBMIDI_MIDI_InitInterface();
#endif /* (USBMIDI_MIDI_EXT_MODE >= USBMIDI_ONE_EXT_INTRF) */
}
...
#if (USBMIDI_MIDI_OUT_BUFF_SIZE > 0)
    /*******************************************************************************
    * Function Name: USBMIDI_MIDI_OUT_Service
    ****************************************************************************//**
    *
    ...
    * \globalvars
    *
    *  \ref USBMIDI_midiOutBuffer: Used as temporary buffer between USB
    ...
    *******************************************************************************/
    ...
    {
    #if (USBMIDI_MIDI_OUT_BUFF_SIZE >= 256)
        uint16 outLength;
        uint16 outPointer;
    #else
        uint8 outLength;
        uint8 outPointer;
    #endif /* (USBMIDI_MIDI_OUT_BUFF_SIZE >= 256) */

        /* Service the USB MIDI output endpoint. */
        if (USBMIDI_OUT_BUFFER_FULL == USBMIDI_GetEPState(USBMIDI_midi_out_ep))
        {
        #if (USBMIDI_MIDI_OUT_BUFF_SIZE >= 256)
            outLength = USBMIDI_GetEPCount(USBMIDI_midi_out_ep);
        #else
            outLength = (uint8)USBMIDI_GetEPCount(USBMIDI_midi_out_ep);
        #endif /* (USBMIDI_MIDI_OUT_BUFF_SIZE >= 256) */

        #if (!USBMIDI_EP_MANAGEMENT_DMA_AUTO)
            #if (USBMIDI_MIDI_OUT_BUFF_SIZE >= 256)
                outLength = USBMIDI_ReadOutEP(USBMIDI_midi_out_ep,
                                                       USBMIDI_midiOutBuffer, outLength);
            #else
                outLength = (uint8)USBMIDI_ReadOutEP(USBMIDI_midi_out_ep,
                                                              USBMIDI_midiOutBuffer, (uint16) outLength);
            #endif /* (USBMIDI_MIDI_OUT_BUFF_SIZE >= 256) */

            #if (USBMIDI_EP_MANAGEMENT_DMA_MANUAL)
                /* Wait until DMA complete transferring data from OUT endpoint buffer. */
                while (USBMIDI_OUT_BUFFER_FULL == USBMIDI_GetEPState(USBMIDI_midi_out_ep))
                {
                }
            ...
            if (outLength >= USBMIDI_EVENT_LENGTH)
            {
                ...
                #if (USBMIDI_MIDI_EXT_MODE >= USBMIDI_ONE_EXT_INTRF)
                    if ((USBMIDI_midiOutBuffer[outPointer] & USBMIDI_CABLE_MASK) ==
                         USBMIDI_MIDI_CABLE_00)
                    {
                        USBMIDI_MIDI1_ProcessUsbOut(&USBMIDI_midiOutBuffer[outPointer]);
                    }
                    else if ((USBMIDI_midiOutBuffer[outPointer] & USBMIDI_CABLE_MASK) ==
                             USBMIDI_MIDI_CABLE_01)
                    {
                    #if (USBMIDI_MIDI_EXT_MODE >= USBMIDI_TWO_EXT_INTRF)
                         USBMIDI_MIDI2_ProcessUsbOut(&USBMIDI_midiOutBuffer[outPointer]);
                    #endif /*  USBMIDI_MIDI_EXT_MODE >= USBMIDI_TWO_EXT_INTRF */
                    }
                    else
                    {
                        ...
                        #ifdef USBMIDI_MIDI_OUT_EP_SERVICE_CALLBACK
                            USBMIDI_MIDI_OUT_EP_Service_Callback();
                        #endif /* USBMIDI_MIDI_OUT_EP_SERVICE_CALLBACK */
                    }
                #endif /* (USBMIDI_MIDI_EXT_MODE >= USBMIDI_ONE_EXT_INTRF) */

                    /* Process any local MIDI output functions */
                    USBMIDI_callbackLocalMidiEvent(USBMIDI_midiOutBuffer[outPointer] & USBMIDI_CABLE_MASK,
                                                            &USBMIDI_midiOutBuffer[outPointer + USBMIDI_EVENT_BYTE1]);
                    outPointer += USBMIDI_EVENT_LENGTH;
                }
            }
...
#if (USBMIDI_MIDI_IN_BUFF_SIZE > 0)
    /*******************************************************************************
    * Function Name: USBMIDI_MIDI_IN_EP_Service
    ****************************************************************************//**
    *
    ...
    *  USBMIDI_MIDI_IN_Service() and from the
    *  USBMIDI_PutUsbMidiIn() function.
    *
    * \globalvars
    *  USBMIDI_midi_in_ep: Used as an IN endpoint number.
    *  USBMIDI_midiInBuffer: Function loads the data from this buffer to
    *    the USB IN endpoint.
    *   USBMIDI_midiInPointer: Cleared to zero when data are sent.
    *
    * \reentrant
    *  No
    *
    *******************************************************************************/
    void USBMIDI_MIDI_IN_EP_Service(void) 
    {
        ...
        if (USBMIDI_midiInPointer != 0u)
        {
            if(USBMIDI_GetEPState(USBMIDI_midi_in_ep) == USBMIDI_EVENT_PENDING)
            {
            #if (USBMIDI_EP_MANAGEMENT_DMA_AUTO)
                USBMIDI_LoadInEP(USBMIDI_midi_in_ep, NULL, (uint16)USBMIDI_midiInPointer);
            #else
                USBMIDI_LoadInEP(USBMIDI_midi_in_ep, USBMIDI_midiInBuffer,
                                                              (uint16) USBMIDI_midiInPointer);
            #endif /* (USBMIDI_EP_MANAGEMENT_DMA_AUTO) */
    ...
    *******************************************************************************/
    void USBMIDI_MIDI_IN_Service(void) 
    {
        ...
        */
        ...
        if (0u == USBMIDI_midiInPointer)
        {
            do
            {
                if (USBMIDI_midiInPointer <= (USBMIDI_MIDI_IN_BUFF_SIZE - USBMIDI_EVENT_LENGTH))
                {
                    /* Check MIDI1 input port for a complete event */
                    m1 = USBMIDI_MIDI1_GetEvent();
                    if (m1 != 0u)
                    {
                        USBMIDI_PrepareInBuffer(m1, (uint8 *)&USBMIDI_MIDI1_Event.msgBuff[0],
                                                                       USBMIDI_MIDI1_Event.size, USBMIDI_MIDI_CABLE_00);
                    }
                }
                ...
                if (USBMIDI_midiInPointer <= (USBMIDI_MIDI_IN_BUFF_SIZE - USBMIDI_EVENT_LENGTH))
                {
                    ...
                    m2 = USBMIDI_MIDI2_GetEvent();
                    if (m2 != 0u)
                    {
                        USBMIDI_PrepareInBuffer(m2, (uint8 *)&USBMIDI_MIDI2_Event.msgBuff[0],
                                                                       USBMIDI_MIDI2_Event.size, USBMIDI_MIDI_CABLE_01);
                    }
                }
            #endif /*  USBMIDI_MIDI_EXT_MODE >= USBMIDI_TWO_EXT_INTRF */
            }
            while((USBMIDI_midiInPointer <= (USBMIDI_MIDI_IN_BUFF_SIZE - USBMIDI_EVENT_LENGTH)) &&
                  ((m1 != 0u) || (m2 != 0u)));
        }
    #endif /* (USBMIDI_MIDI_EXT_MODE >= USBMIDI_ONE_EXT_INTRF) */

        /* Service the USB MIDI input endpoint */
        USBMIDI_MIDI_IN_EP_Service();
    }


    /***************************************************************************
    * Function Name: USBMIDI_PutUsbMidiIn
    ************************************************************************//**
    *
    ...
    *  The USBMIDI_MIDI_IN_Service() function should also be called to
    ...
    * \globalvars
    *
    *  \ref USBMIDI_midi_in_ep: MIDI IN endpoint number used for
    *        sending data.
    *
    *  \ref USBMIDI_midiInPointer: Checked this variable to see if
    ...
    ***************************************************************************/
                                                                ...
                                                                
    {
        ...
        if (USBMIDI_midiInPointer >
                    (USBMIDI_EP[USBMIDI_midi_in_ep].bufferSize - USBMIDI_EVENT_LENGTH))
        {
            USBMIDI_MIDI_IN_EP_Service();
        }
        ...
        if (USBMIDI_midiInPointer <=
                    (USBMIDI_EP[USBMIDI_midi_in_ep].bufferSize - USBMIDI_EVENT_LENGTH))
        {
            if((ic < USBMIDI_EVENT_LENGTH) || (ic >= USBMIDI_MIDI_STATUS_MASK))
            {
                USBMIDI_PrepareInBuffer(ic, midiMsg, ic, cable);
            }
            else /* Only SysEx message is greater than 4 bytes */
            {
                ...
                do
                {
                    ...
                    USBMIDI_PrepareInBuffer(USBMIDI_MIDI_SYSEX, &midiMsg[idx],
                                                     USBMIDI_EVENT_BYTE3, cable);

                    /* Move to next 3 bytes of message */
                    ic  -= USBMIDI_EVENT_BYTE3;
                    idx += USBMIDI_EVENT_BYTE3;
                        ...
                        (USBMIDI_EP[USBMIDI_midi_in_ep].bufferSize - USBMIDI_EVENT_LENGTH))
                    {
                           ...
                           (USBMIDI_EP[USBMIDI_midi_in_ep].bufferSize - USBMIDI_EVENT_LENGTH))
                        {
                            ...
                            retError = USBMIDI_TRUE;
                            break;
                        }
                    }
                }
                while (ic >= USBMIDI_EVENT_BYTE3);

                if (retError == USBMIDI_FALSE)
                {
                    /* Handle end of message: valid size of messages is 0, 1 and 2 */
                    USBMIDI_PrepareInBuffer(USBMIDI_MIDI_EOSEX, &midiMsg[idx], ic, cable);
                }
            }
        }
        else
    ...
    /*******************************************************************************
    * Function Name: USBMIDI_PrepareInBuffer
    ****************************************************************************//**
    *
    ...
    *
    * \globalvars
    *  USBMIDI_midiInBuffer: This buffer is used for saving and combine the
    *    received from UART(s) and(or) generated internally by
    *    USBMIDI_PutUsbMidiIn() function messages.
    *  USBMIDI_midiInPointer: Used as an index for midiInBuffer to
    *     write data.
    *
    * \reentrant
    *  No
    ...
    *******************************************************************************/
                                                                 ...
                                                                 
    {
        ...
        if (ic >= (USBMIDI_MIDI_STATUS_MASK | USBMIDI_MIDI_SINGLE_BYTE_MASK))
        {
            USBMIDI_midiInBuffer[USBMIDI_midiInPointer] = USBMIDI_SINGLE_BYTE | cable;
            USBMIDI_midiInPointer++;
            USBMIDI_midiInBuffer[USBMIDI_midiInPointer] = ic;
            USBMIDI_midiInPointer++;
            USBMIDI_midiInBuffer[USBMIDI_midiInPointer] = 0u;
            USBMIDI_midiInPointer++;
            USBMIDI_midiInBuffer[USBMIDI_midiInPointer] = 0u;
            USBMIDI_midiInPointer++;
        }
        else if((ic < USBMIDI_EVENT_LENGTH) || (ic == USBMIDI_MIDI_SYSEX))
        {
            if(ic == USBMIDI_MIDI_SYSEX)
            {
                USBMIDI_midiInBuffer[USBMIDI_midiInPointer] = USBMIDI_SYSEX | cable;
                USBMIDI_midiInPointer++;
            }
            else if (srcBuffZero < USBMIDI_MIDI_SYSEX)
            {
                USBMIDI_midiInBuffer[USBMIDI_midiInPointer] = (srcBuffZero >> 4u) | cable;
                USBMIDI_midiInPointer++;
            }
            else if (srcBuffZero == USBMIDI_MIDI_TUNEREQ)
            {
                USBMIDI_midiInBuffer[USBMIDI_midiInPointer] = USBMIDI_1BYTE_COMMON | cable;
                USBMIDI_midiInPointer++;
            }
            else if ((srcBuffZero == USBMIDI_MIDI_QFM) || (srcBuffZero == USBMIDI_MIDI_SONGSEL))
            {
                USBMIDI_midiInBuffer[USBMIDI_midiInPointer] = USBMIDI_2BYTE_COMMON | cable;
                USBMIDI_midiInPointer++;
            }
            else if (srcBuffZero == USBMIDI_MIDI_SPP)
            {
                USBMIDI_midiInBuffer[USBMIDI_midiInPointer] = USBMIDI_3BYTE_COMMON | cable;
                USBMIDI_midiInPointer++;
            }
            else
            {
            ...
            USBMIDI_midiInBuffer[USBMIDI_midiInPointer] = srcBuffZero;
            USBMIDI_midiInPointer++;
            USBMIDI_midiInBuffer[USBMIDI_midiInPointer] = srcBuffOne;
            USBMIDI_midiInPointer++;
            USBMIDI_midiInBuffer[USBMIDI_midiInPointer] = srcBuff[2u];
            USBMIDI_midiInPointer++;
        }
        else if (ic == USBMIDI_MIDI_EOSEX)
        {
            switch (eventLen)
            {
                case 0u:
                    USBMIDI_midiInBuffer[USBMIDI_midiInPointer] = USBMIDI_SYSEX_ENDS_WITH1 | cable;
                    USBMIDI_midiInPointer++;
                    USBMIDI_midiInBuffer[USBMIDI_midiInPointer] = USBMIDI_MIDI_EOSEX;
                    USBMIDI_midiInPointer++;
                    USBMIDI_midiInBuffer[USBMIDI_midiInPointer] = 0u;
                    USBMIDI_midiInPointer++;
                    USBMIDI_midiInBuffer[USBMIDI_midiInPointer] = 0u;
                    ...
                    break;
                case 1u:
                    USBMIDI_midiInBuffer[USBMIDI_midiInPointer] = USBMIDI_SYSEX_ENDS_WITH2 | cable;
                    USBMIDI_midiInPointer++;
                    USBMIDI_midiInBuffer[USBMIDI_midiInPointer] = srcBuffZero;
                    USBMIDI_midiInPointer++;
                    USBMIDI_midiInBuffer[USBMIDI_midiInPointer] = USBMIDI_MIDI_EOSEX;
                    ...
                    break;
                case 2u:
                    USBMIDI_midiInBuffer[USBMIDI_midiInPointer] = USBMIDI_SYSEX_ENDS_WITH3 | cable;
                    USBMIDI_midiInPointer++;
                    USBMIDI_midiInBuffer[USBMIDI_midiInPointer] = srcBuffZero;
                    USBMIDI_midiInPointer++;
                    USBMIDI_midiInBuffer[USBMIDI_midiInPointer] = srcBuffOne;
...
*/
#if (USBMIDI_MIDI_EXT_MODE >= USBMIDI_ONE_EXT_INTRF)
    /*******************************************************************************
    * Function Name: USBMIDI_MIDI_InitInterface
    ****************************************************************************//**
    *
    ...
    *
    * \globalvars
    *   USBMIDI_MIDI_Event: initialized to zero.
    *   USBMIDI_MIDI_TxRunStat: initialized to zero.
    *
    *******************************************************************************/
    void USBMIDI_MIDI_InitInterface(void) 
    {
        USBMIDI_MIDI1_Event.length  = 0u;
        USBMIDI_MIDI1_Event.count   = 0u;
        USBMIDI_MIDI1_Event.size    = 0u;
        USBMIDI_MIDI1_Event.runstat = 0u;
        USBMIDI_MIDI1_TxRunStat     = 0u;
        USBMIDI_MIDI1_InqFlags      = 0u;

        /* Start UART block */
        MIDI1_UART_Start();

        /* Change the priority of the UART TX and RX interrupt */
        CyIntSetPriority(MIDI1_UART_TX_VECT_NUM, USBMIDI_CUSTOM_UART_TX_PRIOR_NUM);
        CyIntSetPriority(MIDI1_UART_RX_VECT_NUM, USBMIDI_CUSTOM_UART_RX_PRIOR_NUM);

    #if (USBMIDI_MIDI_EXT_MODE >= USBMIDI_TWO_EXT_INTRF)
        USBMIDI_MIDI2_Event.length  = 0u;
        USBMIDI_MIDI2_Event.count   = 0u;
        USBMIDI_MIDI2_Event.size    = 0u;
        USBMIDI_MIDI2_Event.runstat = 0u;
        USBMIDI_MIDI2_TxRunStat     = 0u;
        USBMIDI_MIDI2_InqFlags      = 0u;

        /* Start second UART block */
        MIDI2_UART_Start();

        /* Change the priority of the UART TX interrupt */
        CyIntSetPriority(MIDI2_UART_TX_VECT_NUM, USBMIDI_CUSTOM_UART_TX_PRIOR_NUM);
        CyIntSetPriority(MIDI2_UART_RX_VECT_NUM, USBMIDI_CUSTOM_UART_RX_PRIOR_NUM);
    #endif /* (USBMIDI_MIDI_EXT_MODE >= USBMIDI_TWO_EXT_INTRF) */
    ...
    #ifdef USBMIDI_MIDI_INIT_CALLBACK
        USBMIDI_MIDI_Init_Callback();
    #endif /* (USBMIDI_MIDI_INIT_CALLBACK) */
    }


    /*******************************************************************************
    * Function Name: USBMIDI_ProcessMidiIn
    ****************************************************************************//**
    *
    ...
    *******************************************************************************/
                                                                ...
                                                                
    {
        ...
        */
        if ((mData & USBMIDI_MIDI_STATUS_BYTE_MASK) != 0u)
        {
            if ((mData & USBMIDI_MIDI_STATUS_MASK) == USBMIDI_MIDI_STATUS_MASK)
            {
                if ((mData & USBMIDI_MIDI_SINGLE_BYTE_MASK) != 0u) /* System Real-Time Messages(single byte) */
                {
                    midiReturn = mData;
                }
                else                              /* System Common Messages */
                {
                    switch (mData)
                    {
                        case USBMIDI_MIDI_SYSEX:
                            rxStat->msgBuff[0u] = USBMIDI_MIDI_SYSEX;
                            rxStat->runstat = USBMIDI_MIDI_SYSEX;
                            rxStat->count = 1u;
                            rxStat->length = 3u;
                            break;
                        case USBMIDI_MIDI_EOSEX:
                            rxStat->runstat = 0u;
                            rxStat->size = rxStat->count;
                            rxStat->count = 0u;
                            midiReturn = USBMIDI_MIDI_EOSEX;
                            break;
                        case USBMIDI_MIDI_SPP:
                            rxStat->msgBuff[0u] = USBMIDI_MIDI_SPP;
                            rxStat->runstat = 0u;
                            rxStat->count = 1u;
                            rxStat->length = 3u;
                            break;
                        case USBMIDI_MIDI_SONGSEL:
                            rxStat->msgBuff[0u] = USBMIDI_MIDI_SONGSEL;
                            rxStat->runstat = 0u;
                            rxStat->count = 1u;
                            rxStat->length = 2u;
                            break;
                        case USBMIDI_MIDI_QFM:
                            rxStat->msgBuff[0u] = USBMIDI_MIDI_QFM;
                            rxStat->runstat = 0u;
                            rxStat->count = 1u;
                            rxStat->length = 2u;
                            break;
                        case USBMIDI_MIDI_TUNEREQ:
                            rxStat->msgBuff[0u] = USBMIDI_MIDI_TUNEREQ;
                            rxStat->runstat = 0u;
                            rxStat->size = 1u;
                            rxStat->count = 0u;
            ...
            else /* Channel Messages */
            {
                ...
                rxStat->count = 1u;
                switch (mData & USBMIDI_MIDI_STATUS_MASK)
                {
                    case USBMIDI_MIDI_NOTE_OFF:
                    case USBMIDI_MIDI_NOTE_ON:
                    case USBMIDI_MIDI_POLY_KEY_PRESSURE:
                    case USBMIDI_MIDI_CONTROL_CHANGE:
                    case USBMIDI_MIDI_PITCH_BEND_CHANGE:
                        rxStat->length = 3u;
                        break;
                    case USBMIDI_MIDI_PROGRAM_CHANGE:
                    case USBMIDI_MIDI_CHANNEL_PRESSURE:
                        rxStat->length = 2u;
                        break;
                    default:
                        rxStat->runstat = 0u;
        ...
        {
            if (rxStat->runstat == USBMIDI_MIDI_SYSEX)
            {
            ...
            else if (rxStat->runstat != 0u)
            {
                ...
                rxStat->count = 2u;
                switch (rxStat->runstat & USBMIDI_MIDI_STATUS_MASK)
                {
                    case USBMIDI_MIDI_NOTE_OFF:
                    case USBMIDI_MIDI_NOTE_ON:
                    case USBMIDI_MIDI_POLY_KEY_PRESSURE:
    ...
    /*******************************************************************************
    * Function Name: USBMIDI_MIDI1_GetEvent
    ****************************************************************************//**
    *
    ...
    *
    * \globalvars
    *  USBMIDI_MIDI1_Event: RX status structure used to parse received
    *    data.
    *
    *******************************************************************************/
            ...
            */
            ...
            #endif /*  ((MIDI1_UART_RXBUFFERSIZE >= 256u) && (CY_PSOC3)) */
                {
                    ...
                    */
                    ...
                    msgRtn = USBMIDI_ProcessMidiIn(rxData,
                                                    (USBMIDI_MIDI_RX_STATUS *)&USBMIDI_MIDI1_Event);
    ...
    /*******************************************************************************
    * Function Name: USBMIDI_MIDI1_ProcessUsbOut
    ****************************************************************************//**
    *
    ...
    *
    * \globalvars
    *  USBMIDI_MIDI1_TxRunStat: This variable used to save the MIDI
    *    status byte and skip to send the repeated status byte in subsequent event.
    *  USBMIDI_MIDI1_InqFlags: The following flags are set when SysEx
    *    message comes.
    *    USBMIDI_INQ_SYSEX_FLAG: Non-Real Time SySEx message received.
    *    USBMIDI_INQ_IDENTITY_REQ_FLAG: Identity Request received.
    ...
    *******************************************************************************/
    ...
    #ifdef USBMIDI_MIDI1_PROCESS_USB_OUT_ENTRY_CALLBACK
        USBMIDI_MIDI1_ProcessUsbOut_EntryCallback();
    #endif /* (USBMIDI_MIDI1_PROCESS_USB_OUT_ENTRY_CALLBACK) */

        cmd = epBuf[USBMIDI_EVENT_BYTE0] & USBMIDI_CIN_MASK;

        if ((cmd != USBMIDI_RESERVED0) && (cmd != USBMIDI_RESERVED1))
        {
            len = USBMIDI_MIDI_SIZE[cmd];
            i = USBMIDI_EVENT_BYTE1;
            /* Universal System Exclusive message parsing */
            if (cmd == USBMIDI_SYSEX)
            {
                if ((epBuf[USBMIDI_EVENT_BYTE1] == USBMIDI_MIDI_SYSEX) &&
                    (epBuf[USBMIDI_EVENT_BYTE2] == USBMIDI_MIDI_SYSEX_NON_REAL_TIME))
                {
                    /* Non-Real Time SySEx starts */
                    USBMIDI_MIDI1_InqFlags |= USBMIDI_INQ_SYSEX_FLAG;
                }
                else
                {
                    USBMIDI_MIDI1_InqFlags &= (uint8)~USBMIDI_INQ_SYSEX_FLAG;
                }
            }
            else if (cmd == USBMIDI_SYSEX_ENDS_WITH1)
            {
                USBMIDI_MIDI1_InqFlags &= (uint8)~USBMIDI_INQ_SYSEX_FLAG;
            }
            else if (cmd == USBMIDI_SYSEX_ENDS_WITH2)
            {
                USBMIDI_MIDI1_InqFlags &= (uint8)~USBMIDI_INQ_SYSEX_FLAG;
            }
            else if (cmd == USBMIDI_SYSEX_ENDS_WITH3)
            {
                /* Identify Request support */
                if ((USBMIDI_MIDI1_InqFlags & USBMIDI_INQ_SYSEX_FLAG) != 0u)
                {
                    USBMIDI_MIDI1_InqFlags &= (uint8)~USBMIDI_INQ_SYSEX_FLAG;
                    if ((epBuf[USBMIDI_EVENT_BYTE1] == USBMIDI_MIDI_SYSEX_GEN_INFORMATION) &&
                        (epBuf[USBMIDI_EVENT_BYTE2] == USBMIDI_MIDI_SYSEX_IDENTITY_REQ))
                    {
                        /* Set the flag about received the Identity Request.
                        *  The Identity Reply message may be send by user code.
                        */
                        USBMIDI_MIDI1_InqFlags |= USBMIDI_INQ_IDENTITY_REQ_FLAG;
                    }
                }
            }
            ...
            if ((cmd >= USBMIDI_NOTE_OFF) && (cmd <= USBMIDI_PITCH_BEND_CHANGE))
            {
                if (USBMIDI_MIDI1_TxRunStat == epBuf[USBMIDI_EVENT_BYTE1])
                {
                ...
                else
                {
                    ...
                    USBMIDI_MIDI1_TxRunStat = epBuf[USBMIDI_EVENT_BYTE1];
                }
            }
            else
            {
    ...
    #ifdef USBMIDI_MIDI1_PROCESS_USB_OUT_EXIT_CALLBACK
        USBMIDI_MIDI1_ProcessUsbOut_ExitCallback();
    #endif /* (USBMIDI_MIDI1_PROCESS_USB_OUT_EXIT_CALLBACK) */
    }


#if (USBMIDI_MIDI_EXT_MODE >= USBMIDI_TWO_EXT_INTRF)
    /*******************************************************************************
    * Function Name: USBMIDI_MIDI2_GetEvent
    ****************************************************************************//**
    *
    *  Checks for incoming MIDI data, calls the MIDI event builder if so.
    *  Returns either empty or with a complete event.
    ...
    *
    * \globalvars
    *  USBMIDI_MIDI2_Event: RX status structure used to parse received
    *    data.
    *
    *******************************************************************************/
            ...
            */
            ...
            #endif /* ((MIDI2_UART_RXBUFFERSIZE >= 256u) && (CY_PSOC3)) */
                {
                    ...
                    */
                    ...
                    msgRtn = USBMIDI_ProcessMidiIn(rxData,
                                                    (USBMIDI_MIDI_RX_STATUS *)&USBMIDI_MIDI2_Event);
    ...
    /*******************************************************************************
    * Function Name: USBMIDI_MIDI2_ProcessUsbOut
    ****************************************************************************//**
    *
    *  Process a USB MIDI output event.
    *  Puts data into the MIDI TX output buffer.
    ...
    *
    * \globalvars
    *  USBMIDI_MIDI2_TxRunStat: This variable used to save the MIDI
    *    status byte and skip to send the repeated status byte in subsequent event.
    *  USBMIDI_MIDI2_InqFlags: The following flags are set when SysEx
    *    message comes.
    *  USBMIDI_INQ_SYSEX_FLAG: Non-Real Time SySEx message received.
    *  USBMIDI_INQ_IDENTITY_REQ_FLAG: Identity Request received.
    *    This bit should be cleared by user when Identity Reply message generated.
    ...
    *******************************************************************************/
                                                            ...
                                                            
    {
    ...
    #ifdef USBMIDI_MIDI2_PROCESS_USB_OUT_ENTRY_CALLBACK
        USBMIDI_MIDI2_ProcessUsbOut_EntryCallback();
    #endif /* (USBMIDI_MIDI2_PROCESS_USB_OUT_ENTRY_CALLBACK) */
        ...
        if ((cmd != USBMIDI_RESERVED0) && (cmd != USBMIDI_RESERVED1))
        {
            ...
            if(cmd == USBMIDI_SYSEX)
            {
                ...
                {
                    /* SySEx starts */
                    USBMIDI_MIDI2_InqFlags |= USBMIDI_INQ_SYSEX_FLAG;
                }
                else
                {
                    USBMIDI_MIDI2_InqFlags &= (uint8)~USBMIDI_INQ_SYSEX_FLAG;
                }
            }
            else if(cmd == USBMIDI_SYSEX_ENDS_WITH1)
            {
            ...
            {
                /* Identify Request support */
                if ((USBMIDI_MIDI2_InqFlags & USBMIDI_INQ_SYSEX_FLAG) != 0u)
                {
                    USBMIDI_MIDI2_InqFlags &= (uint8)~USBMIDI_INQ_SYSEX_FLAG;
                    ...
                    {   /* Set the flag about received the Identity Request.
                        *  The Identity Reply message may be send by user code.
                        */
                        USBMIDI_MIDI2_InqFlags |= USBMIDI_INQ_IDENTITY_REQ_FLAG;
                    }
                }
            }
            else /* Do nothing for other command */
            ...
            if ((cmd >= USBMIDI_NOTE_OFF) && ( cmd <= USBMIDI_PITCH_BEND_CHANGE))
            {
                if (USBMIDI_MIDI2_TxRunStat == epBuf[USBMIDI_EVENT_BYTE1])
                {   /* Skip the repeated Status byte */
                    i++;
                }
                else
                {   /* Save Status byte for next event */
                    USBMIDI_MIDI2_TxRunStat = epBuf[USBMIDI_EVENT_BYTE1];
                }
            }
            else
            {   /* Clear Running Status */
    ...
    #ifdef USBMIDI_MIDI2_PROCESS_USB_OUT_EXIT_CALLBACK
        USBMIDI_MIDI2_ProcessUsbOut_ExitCallback();
    #endif /* (USBMIDI_MIDI2_PROCESS_USB_OUT_EXIT_CALLBACK) */
    }
#endif /* (USBMIDI_MIDI_EXT_MODE >= USBMIDI_TWO_EXT_INTRF) */
#endif /* (USBMIDI_MIDI_EXT_MODE >= USBMIDI_ONE_EXT_INTRF) */
...
#endif  /* defined(USBMIDI_ENABLE_MIDI_STREAMING) */
